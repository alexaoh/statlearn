...
Without writing all the calculations here, the predictions are as follows. 

#### K = 1

When K = 1 the closest point to the test point is the third observation. Since this observation belongs to class, $(1,2)$ will also be classified as class A. 
...
K = 7 is a bad choice because all the observations we have to our disposal is used at once. This is a bad tactic because the predicted value will be the same (class B) no matter which point on the board we want to classify. This will not give any more predictive or useful information. 
...
The linear function in $\boldsymbol{x}$ can be obtained by inserting the estimations from earlier, joined by an estimation of the prior probabilities. 
...
$$
\hat{x_1} = \frac{1}{\hat{\beta_1}}(\log{\left(\frac{p}{1-p}\right)} - \hat{\beta_0} - \hat{\beta_2}x_2) = \frac{1}{0.05}(\log{\left(\frac{0.5}{1-0.5}\right)} - (-6) - 1\cdot 3.5) = 50 \hspace{1mm}\text{hours}
$$
...
The sensitivity of the test is the proportion of correctly classified positive observations. More precisely, the number of true positives (number of predicted diseased that are actually diseased) divided by the actual positives in the population.

The specificity of the test is the proportion of correctly classified negative observations. More precisely, the number of true negatives (number of predicted non-diseased that are actually non-diseased) divided by the actual negatives in the population.  
...
table(knn.model, Weekly_test$Direction)
```
